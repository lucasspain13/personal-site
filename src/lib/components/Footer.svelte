...
{#if visible}
	<footer class="bg-base-200 text-base-content pt-10 pb-4">
		<div class="container mx-auto px-4">
			<div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
				...
				<div in:fly={{ y: 20, duration: 500, delay: 100 }}>
					...
					<p class="mb-4 opacity-80">
						Software Engineer specializing in web development, application development, machine
						learning operations, and infrastructure operations.
					</p>
					...
					<div class="flex space-x-4 mt-4">
						{#each socialLinks as link, i}
							<a
								...
								class="btn btn-circle btn-ghost hover:text-primary transition-colors duration-300"
					...
					<div in:fly={{ y: 20, duration: 500, delay: 200 + i * 100 }}>
						...
						<ul class="space-y-2">
							{#each section.links as link, j}
								<li in:fade={{ delay: 300 + j * 50, duration: 200 }}>
									<a
										href={link.url}
										class="hover:text-primary transition-colors duration-300 opacity-80 hover:opacity-100"
									>
										{link.name}
									</a>
								</li>
			...
			<!-- Copyright -->
			<div
				class="border-t border-base-300 pt-4 text-center text-sm opacity-70"
				in:fade={{ delay: 500, duration: 300 }}
			>
				<p>© {new Date().getFullYear()} Your Name. All rights reserved.</p>
				<p class="mt-2">Built with Svelte, TailwindCSS, and DaisyUI</p>
			</div>
		</div>
	</footer>
{/if}
